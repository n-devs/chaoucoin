...
const char * const BITCOIN_CONF_FILENAME = "chaoucoin.conf";
...
#else
    const char* pszModule = "chaoucoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Chaoucoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Chaoucoin";
#else
    // Unix
    return pathRet / ".chaoucoin";
#endif
#endif
}
